...
int main()
{
  ...
  XImage *img = XShmCreateImage(display,visual,24,ZPixmap,NULL,&shminfo,width,height);

  shminfo.shmid = shmget(IPC_PRIVATE,img->bytes_per_line * img->height,IPC_CREAT|0777);

  shminfo.shmaddr = img->data = data = shmat(shminfo.shmid,0,0);
  ...
  while(running)
  {
    ...
    XShmPutImage(display,win,DefaultGC(display,screen_num),img,0,0,0,0,width,height,False);
    clear();
    t0 = t1;
    }
  }
